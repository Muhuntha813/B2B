...
const Machinery = () => {
  ...
  // Filter options
  const filterOptions = {
    categories: [...new Set(machinesData.map(machine => machine.category))],
    grades: [...new Set(machinesData.map(machine => machine.condition))], // Using conditions as grades for machinery
    priceRanges: [
                ...
                {currentMachines.map((machine) => (
                  <div key={machine.id} className="group relative">
                    <ProductCard
                      ...
                      type="machinery"
                    />
                    {/* Contact Seller Button - Always visible on mobile, hover on desktop */}
                    <div className="mt-3 sm:mt-4 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200">
                      <button
                        onClick={() => handleContactSeller(machine.id, machine.supplier)}
                        className="w-full bg-gradient-to-r from-primary-600 to-secondary-600 text-white py-2.5 sm:py-3 px-4 rounded-lg font-medium hover:from-primary-700 hover:to-secondary-700 transition-all duration-200 shadow-md hover:shadow-lg flex items-center justify-center space-x-2 text-sm sm:text-base"
                      >
