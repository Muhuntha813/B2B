...
const Jobs = () => {
  ...
  const [requestingChat, setRequestingChat] = useState(null)
  const itemsPerPage = 10

  const handleChatRequest = async (jobId, jobTitle) => {
    if (!currentUser) {
      alert('Please login to request chat access')
      navigate('/login')
      return
    }

    setRequestingChat(jobId)
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/chat/request`, {
        ...
        body: JSON.stringify({
          ...
          job_id: jobId,
          reason: `Requesting chat access for job: ${jobTitle}`
        })
      })

      if (response.ok) {
        alert('Chat request sent to admin. You will be notified once approved.')
      } else {
        const errorData = await response.json()
        alert(errorData.error || 'Failed to send chat request')
      }
    } catch (error) {
  ...
  const filterOptions = {
    ...
    status: [...new Set(jobs.map(job => normalizeStatus(job.status)).filter(Boolean))],
  ...
  useEffect(() => {
    ...
    // Status filter (map 'active' -> 'Open')
    if (filters.status.length > 0) {
      filtered = filtered.filter(job => filters.status.includes(normalizeStatus(job.status)))
    }
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(normalizeStatus(job.status))}`}>
                    ...
                    <div className="flex justify-between items-center">
                      <div className="text-sm text-gray-500">
                        <span className="mr-4">
                          ...
                          <span>
                          Posted {job.posted_date ? new Date(job.posted_date).toLocaleDateString() : '-'}
                        </span>
                      </div>
                      <div className="flex flex-wrap gap-2">
                        <Link 
                          to={`/job/${job.id}`}
                          className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-sm inline-flex items-center space-x-1"
                        >
                        ...
                        {(job.status === 'Open' || job.status === 'open') && (
                          <button
                            onClick={() => handleChatRequest(job.id, job.title)}
                            disabled={requestingChat === job.id}
                            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm inline-flex items-center space-x-1"
                          >
                            {requestingChat === job.id ? (
                              <>
