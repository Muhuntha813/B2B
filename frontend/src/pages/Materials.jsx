...
const Materials = () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const mappedData = data.map(p => ({
            ...
            grade: p.category || 'Standard', // Using category as grade fallback
            price: Number(p.price),
            unit: 'kg', // Default unit
            image: p.image || '/images/hdpe-natural.svg',
            location: 'Multiple', // Default location
            supplier: 'Various Suppliers',
            rating: 4.5, // Default rating
            inStock: true,
            description: `${p.name} - ${p.category || 'General category'}`
          }))
  ...
  // Filter categories and grades based on loaded data
  const categories = [...new Set(products.map(item => item.category).filter(Boolean))]
  const grades = [...new Set(products.map(item => item.grade).filter(Boolean))]
