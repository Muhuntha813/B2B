...
const Chat = () => {
  ...
  // Server-backed polling for messages when token available
  useEffect(() => {
    if (!useServerChat || !currentUser || !recipientId) return
    let timer = null
    const fetchThread = async () => {
      try {
        const API_BASE_URL = getApiBaseUrlDynamic();
        const res = await fetch(`${API_BASE_URL}/messages/thread?user_uid=${currentUser.uid}&peer_uid=${recipientId}`, {
          headers: { Authorization: `Bearer ${token}` }
        })
        const data = await res.json()
        if (Array.isArray(data)) setMessages(data.map(m => ({ id: m.id, text: m.body, senderId: m.sender_id, timestamp: m.created_at })))
      } catch (e) {
  ...
  }, [useServerChat, currentUser, recipientId, token])
      ...
      if (document.visibilityState === 'visible') {
        try {
          ...
          await fetch(`${API_BASE_URL}/messages/read`, {
            ...
            body: JSON.stringify({ user_uid: currentUser.uid, peer_uid: recipientId })
          })
        } catch {}
      }
    }
  ...
  const sendMessage = async (e) => {
    ...
    setSending(true)
    try {
      if (useServerChat && recipientId) {
        const API_BASE_URL = getApiBaseUrlDynamic();
        const response = await fetch(`${API_BASE_URL}/messages`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ sender_uid: currentUser.uid, receiver_uid: recipientId, body: message.trim() })
        })
        
        if (!response.ok) {
          const errorData = await response.json()
          alert(errorData.error || 'Failed to send message. Please check if you have chat permission approved.')
          return
        }
      } else {
        // Fallback: Firebase
        ...
        await updateDoc(doc(db, 'conversations', selectedConversation.id), { lastMessage: message.trim(), lastMessageTime: serverTimestamp() })
      }

      setMessage('')
    } catch (error) {
      console.error('Error sending message:', error)
      alert('Failed to send message. Please check if you have chat permission approved.')
    } finally {
      setSending(false)
    }
  }
