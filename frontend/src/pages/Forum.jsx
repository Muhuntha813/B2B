...
const Forum = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/forum/posts?page=1&pageSize=20&search=${encodeURIComponent(searchTerm)}`)
  ...
  const forumPosts = posts.length ? posts.map(p => ({
    id: p.id,
    avatar: (p.user_id || '?').toString().slice(0,2).toUpperCase(),
    name: `User ${p.user_id}`,
    category: 'General',
    title: p.title,
    content: p.content,
                  ...
                  onSubmit={async (e) => {
                    ...
                    try {
                      ...
                      const res = await fetch(`${API_BASE_URL}/forum/posts`, {
                        ...
                        body: JSON.stringify({ user_uid: currentUser.uid, title: newTitle, content: newContent })
                      })
                      if (res.ok) {
                        setNewTitle(''); setNewContent(''); setCreating(false)
                        // optimistic reload
                        const data = await res.json()
                        setPosts([{ id: data.id, user_id: 0, title: newTitle, content: newContent, created_at: new Date().toISOString() }, ...posts])
                      }
                    } catch (e) {}
                  }}
                  className="mt-4 space-y-3"
                >
                  <input value={newTitle} onChange={(e)=>setNewTitle(e.target.value)} placeholder="Title" className="w-full px-3 py-2 border rounded" />
                  <textarea value={newContent} onChange={(e)=>setNewContent(e.target.value)} placeholder="Content" rows={4} className="w-full px-3 py-2 border rounded" />
